...
class AddCategoriaTask(val context: Context, val name : String, val status:Boolean,val mes : Int) : AsyncTask<Void, Void, String>(){


    override fun doInBackground(vararg params: Void?): String {
        try{
            val db = DbBuilder.getInstance(context)
            db.categoriaDAO().armazenar(Categoria(name, status, mes))
        }catch (ex: Exception){
